...
**A second follow-up added the last piece of this whole effort: reading
`INPUT_PATH` from stdin.** Prompted by continuing to look for what else
would make this CLI more agent-friendly - an agent that already has data
in memory (fetched from an API, generated by another tool, piped from a
shell command) previously had no way to hand it to sniff-rs without
writing it to a real file itself first, exactly the "easy to use non-
interactively" friction this whole effort exists to remove, and the same
Unix convention this tool's own `OUTPUT_PATH` already uses for stdout.

`sniff-rs -` (literally `-` as `INPUT_PATH`) now works: `resolve_stdin_
input` buffers the *entire* contents of stdin into a real, seekable
temporary file (the same `TempFile` abstraction `decompress_if_needed`
already uses) before any reader ever runs. This is deliberate, not
incidental - every reader in this project needs genuine random file
access somewhere (Parquet's own tail-anchored footer, SQLite's page-by-
page b-tree walk, the zip-based formats' central directory, `--sql-mode
inline`'s own real *second* pass over the same file once column types
are known), which a raw pipe can never give directly. Buffering once up
front is what lets every existing reader - all ~35 of them - work
completely unchanged, with zero per-format stdin-specific code, rather
than needing its own streaming special case. `--format` still works
exactly as it does for a real file (still required for a format with no
fixed magic-number/leading-character signature - CSV/TSV/TOML/YAML/INI,
same as any other extensionless input already needs), and content-based
sniffing (`sniff_format`) still works too, since it's already just a
...
everywhere else in this tool derives a filename from the *input* path
itself, which a literal `-` has none of. `--load-into` is the one
exception: it already forbids an output path of its own (the generated
SQL streams straight into the target engine, never a named file), so the
"needs somewhere real to go" concern this check exists for genuinely
doesn't apply there - `sniff-rs - --format csv --output-format sql
--load-into sqlite:mydb.db < data.csv` works with no `OUTPUT_PATH` at
all, verified directly against a real, installed SQLite build (the
loaded table's own row count matched the source data exactly).

**A real, non-obvious bug was caught before this shipped, not found
after**: the first working version correctly buffered stdin into a real
temp file, but that temp file's own randomly-generated name (`sniff-rs-
<pid>-<nanos>-<n>.tmp`) leaked straight into the actual output content -
`dispatch_reader`'s own default single-table name comes from `logical_
path`'s file stem, and `logical_path` was, at that point, simply the
temp file's own real path. A quick manual check (piping a real CSV
fixture through and reading the JSON output by hand, rather than trusting
it compiled and ran without erroring) caught this immediately: the
table's own key in the output was something like `"sniff-rs-3199-
1789544843929614000-0"` instead of anything meaningful. Fixed by
overriding `logical_path` to the literal string `"stdin"` specifically
when the real `input_path` is `-` - `read_path` (the path every reader
actually opens) is untouched and still correctly points at the real temp
file; only the *display*/*naming* path changes. The `"file"` field in
JSON/Markdown output still correctly reports the literal `"-"` the user
actually typed (derived from `args.input_path` directly, never touched
by this bug or its fix), so the two together give an honest, meaningful
answer: `"file": "-"`, table name `"stdin"`.

Verified against a real release binary via genuine subprocess stdin
piping (`Command::new(bin()).stdin(Stdio::piped())`, writing real bytes
to the child's stdin - the only reliable way to test this, since an
in-process unit test reading `std::io::stdin()` directly would read
whatever `cargo test`'s own stdin happens to be, shared unpredictably
across however many tests run in parallel): a real CSV fixture piped
through with `--format csv` produces a table named `"stdin"`, not the
scratch temp filename; a JSON payload piped with no `--format` at all
correctly content-sniffs; CSV piped with no `--format` correctly still
demands one, the identical rule an extensionless real file already has;
`sniff-rs -` with no `OUTPUT_PATH` fires the disclosed error; and
`--output-format sql`'s own inline mode was confirmed to survive its
real second pass over the buffered temp file - not just a correct
header/schema, but a real value from the fixture's own first data row
surviving into the generated `INSERT` statement, proving the re-read
genuinely re-reads real content rather than an already-exhausted stream.
One new unit test (`a_real_path_passes_through_unchanged_with_no_temp_
file`, covering the non-stdin passthrough - the one half of this function
safe and meaningful to test in-process) plus five new integration tests
